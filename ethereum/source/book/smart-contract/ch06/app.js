...
var web3 = new Web3(new Web3.providers.HttpProvider("http://54.180.40.81:9540"));
  ...
  web3.currentProvider.sendAsync({
    ...
    if (result.result.pending) {
      if (result.result.pending[address]) {
        ...
        for (var txn in txns) {
          cost = cost.add((new BigNumber(parseInt(txns[txn].value))).add((new BigNumber(parseInt(txns[txn].gas)))
          .mul(new BigNumber(parseInt(txns[txn].gasPrice)))));
        }
...
app.get("/deploy", function (req, res) {
  ...
  for (var contractName in contracts) {
    ...
    var data = contract.new.getData.call(null, ...arguments, {data: byteCode});
    var gasRequired = web3.eth.estimateGas({data: "0x" + data});
    web3.eth.getBalance(address, function (err, balance) {
      var etherAvailable = web3.fromWei(balance, "ether");
      etherSpentInPendingTransactions(address, function (err, balance) {
