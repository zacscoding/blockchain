...
public class BlockHeader {
    ...
    // TEMP
    public byte[] getEncodedTemp(byte[] s1, byte[] s2) {
        byte[] parentHash = RLP.encodeElement(this.parentHash);
        ...
        byte[] encodedS1 = RLP.encodeElement(s1);
        byte[] encodedS2 = RLP.encodeElement(s2);

        return RLP.encodeList(parentHash, unclesHash, coinbase,
            stateRoot, txTrieRoot, receiptTrieRoot, logsBloom, difficulty, number,
            gasLimit, gasUsed, timestamp, extraData, encodedS1, encodedS2);
    }

    public byte[] getEncodedTemp2(byte[] s1, long s2) {
        byte[] parentHash = RLP.encodeElement(this.parentHash);
        ...
        byte[] encodedS1 = RLP.encodeElement(s1);
        byte[] encodedS2 = RLP.encodeBigInteger(BigInteger.valueOf(s2));
